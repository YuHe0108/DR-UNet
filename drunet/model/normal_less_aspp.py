...
def aspp_block(inputs, filters, out_filters, kernel_size):
    ...
    out_rate_1 = conv_block(inputs, filters // 4, kernel_size, dilate_rate=1)
    out_rate_2 = conv_block(inputs, filters // 4, kernel_size, dilate_rate=2)
    out_rate_4 = conv_block(inputs, filters // 4, kernel_size, dilate_rate=4)
    out_rate_8 = conv_block(inputs, filters // 4, kernel_size, dilate_rate=8)
    # 特征图拼接
...
def attention_block(inputs, filters):
    c_out = layers.Multiply()([CBAM.channel_attention_conv(inputs, filters), inputs])
    out = layers.Multiply()([CBAM.SpatialAttention()(c_out), c_out])
    return out
...
def my_model(input_shape, num_classes=1, dims=32, kernel_size=3):
    ...
    conv_dims_list = [32, 64, 128, 192, 256, 320]
    out_dims_list = [64, 128, 192, 256, 320, 384, 448, 512, 640, 768, 896, 1024, 1280]
    ...
    up_1 = muti_conv_residual_block(
        up_1, out_dims_list[0], conv_dims_list[0])
    # 特征拼接
