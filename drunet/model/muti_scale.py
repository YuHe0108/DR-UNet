...
def conv_block(inputs,
               ...
               cbam=False):
    """conv+bn+relu"""
    out = conv_same_layer(inputs, filters, kernel_size, strides, dilate_rate=dilate_rate, use_bias=not bn)
    if bn:
...
def my_model(input_shape, num_classes=1, dims=32, kernel_size=3):
    ...
    out_rate_1 = conv_block(out_32_r, conv_dims_list[4], kernel_size, dilate_rate=1)
    out_rate_2 = conv_block(out_32_r, conv_dims_list[4], kernel_size, dilate_rate=2)
    out_rate_4 = conv_block(out_32_r, conv_dims_list[4], kernel_size, dilate_rate=4)
    out_rate_8 = conv_block(out_32_r, conv_dims_list[4], kernel_size, dilate_rate=8)
    # 特征图拼接
    concat_out = layers.Concatenate()([out_32_r, out_rate_1, out_rate_2, out_rate_4, out_rate_8])
    # 整合拼接的特征图
    out = conv_block(concat_out, filters=out_dims_list[4], kernel_size=1)
    center_u_1 = layers.UpSampling2D(2, interpolation='bilinear')(out)

    # 第二个中间层
    out_rate_1 = conv_block(out, conv_dims_list[4], kernel_size, dilate_rate=1)
    out_rate_2 = conv_block(out, conv_dims_list[4], kernel_size, dilate_rate=2)
    out_rate_4 = conv_block(out, conv_dims_list[4], kernel_size, dilate_rate=4)
    out_rate_8 = conv_block(out, conv_dims_list[4], kernel_size, dilate_rate=8)
    # 特征图拼接
    concat_out = layers.Concatenate()([out, out_rate_1, out_rate_2, out_rate_4, out_rate_8])
    # 整合拼接的特征图
    out = conv_block(concat_out, filters=out_dims_list[4], kernel_size=1)
    center_u_2 = layers.UpSampling2D(2, interpolation='bilinear')(out)
    ...
    up_16_c = layers.Concatenate()([up_16, out_16_r, center_u_1, center_u_2, out_16_u])  # [16, 16, 640*4]
    ...
    up_8_c = layers.Concatenate()([up_8, out_8_r, out_8_u])
    ...
    return keras.Model(inputs, out, name='my_model_muti_scale')
